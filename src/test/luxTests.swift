...
class InstrumentationFunctionalityTest: XCTestCase {
    ...
    final func testLuxReportsCorrectly() {
        if let testScript = self.testScriptPath, let coverallsOutput = self.coverallsOutputString {
            let x = executeAndParseTestCommand(testScript)!
                >>- report(imap: InstrumentationMap())
                >>- reportCoveralls(RepositoryReferenceStrategy.Token(repoToken: "token"))
            //XCTAssertEqual(x.value!, coverallsOutput)
        }
    }
}
